...
Run from repo root: python scripts/step04_build_target.py
"""
...
LONG_PARQUET = os.path.join(DATA_DIR, "rand_hrs_long_sample.parquet")
OUT_PARQUET = os.path.join(DATA_DIR, "rand_hrs_long_with_target.parquet")
...
def main() -> None:
    if not os.path.isfile(LONG_PARQUET):
        print(
            f"ERROR: Long parquet not found: {LONG_PARQUET}",
            file=sys.stderr,
        )
        print("Run step03_reshape_to_long.py first.", file=sys.stderr)
        sys.exit(1)
    ...
    out_model = os.path.join(DATA_DIR, "rand_hrs_model_sample.parquet")
    df.to_parquet(out_model, index=False)
