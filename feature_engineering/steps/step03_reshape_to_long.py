...
Run from repo root: python -m feature_engineering.steps.step03_reshape_to_long
"""
...
from feature_engineering.pipeline_config import get_sample_rows

REPO_DIR = Path(__file__).resolve().parents[2]
SAS_FILE = REPO_DIR / "data" / "randhrs1992_2022v1_SAS" / "randhrs1992_2022v1.sas7bdat"
...
def main() -> None:
    ...
    if sample_rows == 0:
        ...
        df_wide, _ = pyreadstat.read_sas7bdat(str(SAS_FILE))
    else:
        print(f"Loading first {sample_rows} rows from {SAS_FILE} ...")
        df_wide, _ = pyreadstat.read_sas7bdat(str(SAS_FILE), row_limit=sample_rows)
    ...
    out_path = OUT_DIR / "rand_hrs_long_sample.parquet"
    df_long.to_parquet(str(out_path), index=False)
