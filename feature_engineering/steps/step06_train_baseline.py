...
Run from repo root: python -m feature_engineering.steps.step06_train_baseline
"""
...
def main() -> None:
    if not X_PATH.is_file() or not Y_CONDITIONS_PATH.is_file():
        print(
            f"ERROR: Need {X_PATH} and {Y_CONDITIONS_PATH} (run step05_build_features).",
            file=sys.stderr,
        )
        sys.exit(1)
    ...
    X = pd.read_parquet(str(X_PATH))
    y_all = pd.read_parquet(str(Y_CONDITIONS_PATH))
    ...
    for suffix, name in conditions:
        ...
        out_path = MODEL_DIR / f"baseline_run_{suffix}.json"
        with open(out_path, "w") as f:
            json.dump(run_record, f, indent=2)
        print(f"  Saved: {out_path}")

    # Backward compatibility: diabetes-only run as baseline_run.json
    diabe_record_path = MODEL_DIR / "baseline_run_DIABE.json"
    out_baseline = MODEL_DIR / "baseline_run.json"
    if diabe_record_path.is_file():
