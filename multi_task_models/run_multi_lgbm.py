"""
Multi-output LightGBM for all 8 health conditions.

Trains separate LGBMClassifier for each condition with shared features,
logs all results to MLflow under single run.

Run from repo root:
  python multi_task_models/run_multi_lgbm.py
...
def main() -> None:
    """Run multi-task LightGBM for all 8 conditions."""
    print("=" * 70)
    print("Multi-Task LightGBM — All 8 Health Conditions")
    print("=" * 70)
    ...
    print("\nPer-task class weights (for class_weight='balanced'):")
    for suffix, weight in weights.items():
        print(f"  {suffix}: {weight:.2f}")
    
    # LightGBM hyperparameters from env or defaults
    max_depth = int(os.environ.get("LGBM_MAX_DEPTH", "6"))
    learning_rate = float(os.environ.get("LGBM_LEARNING_RATE", "0.05"))
    n_estimators = int(os.environ.get("LGBM_N_ESTIMATORS", "100"))
    
    print(
        f"\nLightGBM params: max_depth={max_depth}, lr={learning_rate}, "
    ...
    run_name = f"multi_lgbm_{len(feature_columns)}feat_{time_id}"
    
    mlflow.set_experiment("hea_multitask")
    with mlflow.start_run(run_name=run_name):
        # Model-level params
        mlflow.log_param("model_type", "multi_output_lgbm")
        ...
        mlflow.log_param(
            "feature_selection", "importances" if importances_path else "all"
        )
        if min_importance is not None:
    ...
    results_path = MODEL_DIR / f"multi_lgbm_results_{time_id}.json"
    
    output = {
        "timestamp": time_id,
        "model_type": "multi_output_lgbm",
        "n_tasks": len(results),
    ...
    for target_col, model in models.items():
        ...
        imp_path = (
            MODEL_DIR / f"multi_lgbm_{suffix}_importances_{time_id}.csv"
        )
